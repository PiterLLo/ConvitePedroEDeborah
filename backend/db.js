    ...
    db.get(
        ...
        (err, row) => {
            ...
            if (row) {
                ...
                db.run(
                    ...
                    (err) => {
                        ...
                        db.run(`DROP TABLE rsvp`, (err) => {
                            ...
                            // Renomeia nova para rsvp
                            db.run(
                                `ALTER TABLE rsvp_new RENAME TO rsvp`,
                                (err) => {
                                    if (err) return console.error(err);
                ...
                // Nenhuma tabela antiga → só renomeia
                db.run(`ALTER TABLE rsvp_new RENAME TO rsvp`, (err) => {
                    if (err) return console.error(err);
                    console.log("Tabela rsvp criada com created_at.");
                });
            }
        }
    );
